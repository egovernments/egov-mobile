...
public class BaseActivity extends AppCompatActivity implements Interceptor.ErrorListener, LanguageChangeListener {
    ...
    private static final int REQUEST_CHECK_SETTINGS = 124;
    ...
    public void showSnackBarWithTime(String msg, int time) {
        Snackbar snackbar = Snackbar.make(findViewById(android.R.id.content), msg,
                Snackbar.LENGTH_LONG).setDuration(time);
        View snackbarView = snackbar.getView();
        TextView tv = (TextView) snackbarView.findViewById(android.support.design.R.id.snackbar_text);
        tv.setMaxLines(5);
        snackbar.show();
    ...
    public void errorOccurred(String errorMsg, int errorCode) {
        ...
        if (errorMsg.length() > 60)
            duration = 4000;
        showSnackBarWithTime(errorMsg, duration);
    }
